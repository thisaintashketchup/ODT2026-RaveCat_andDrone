...
while True:
    if value == '1':
        ...
        def strobe_task(): # CLAUDE SOLN 1
            global strobe_running
            strobe_running = True
            for i in range (0, 10000000, 1):
                if not strobe_running:
                    break# strobe light on pwm with original oiia movements.
                strobe.duty((random.randint(20,800)))
                time.sleep(0.02)
                strobe.duty((random.randint(800,1020)))
                time.sleep(0.1)
...
#                 count_intensity += 1
            ...
            print("count intensity:" + str(count_intensity))
            print("seconds elapsed:" + str(i))
            if len(user_oiia) < len(og_oiia):  # only append actual presses, up to 28
...
#asyncio.run(main())
        ...
        for i in range(min(len(og_oiia), len(user_oiia))): #fixed by claude.  og: for i in range(len(og_oiia)-1, 1):
            ...
            if (diff == 0):
                correct_responses += 1                
            elif diff < 100:#need to figure out this error spectrum            #2 digit gap
                correct_responses += 0.7
            elif diff < 1000: #3 digit gap
                correct_responses += 0.4
            elif diff < 10000: # 4 digit gap
                correct_responses += 0.2 
            else:
                correct_responses += 0.1 # removed error cushion 
        #correct_responses += (humanerror_count) #human error / deflection sort of evened out?
